Describe "Validating the module manifest" {
	$moduleRoot = (Resolve-Path "$global:testroot\..\SecretManagement.HashTableBug").Path
	$manifest = ((Get-Content "$moduleRoot\SecretManagement.HashTableBug.psd1") -join "`n") | Invoke-Expression
