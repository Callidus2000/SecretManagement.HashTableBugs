...
function Get-SecretInfo {
    ...
    $allSecretInformation = @('Foo', 'Foo', 'bar') | ForEach-Object {
        [Microsoft.Powershell.SecretManagement.SecretInformation]::new(
            ...
            @{
                Comment = "This is not very secret ;-)"
            }        # The metadata of our vault as expressed as a hashtable.
        )
    }
    ...
    if ($Filter) {
        $secretInfoMatch = $allSecretInformation | Where-Object Name -Like $Filter
    }
    else {
        $secretInfoMatch = $allSecretInformation
    }
    Write-Host -Fore Cyan 'Found the following Info:'
    Write-Host -Fore Cyan "$($secretInfoMatch|Format-Table|Out-String)"
    return $secretInfoMatch
    # #Read in our CSV
